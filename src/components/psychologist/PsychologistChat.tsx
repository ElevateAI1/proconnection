...
export const PsychologistChat = ({ patientId, patientName, patientImage }: PsychologistChatProps) => {
  const { user } = useAuth();
  ...
  const displayName = patientName || (patient ? `${patient.first_name} ${patient.last_name}` : 'Paciente');
